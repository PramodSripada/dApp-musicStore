...
function loadCreator() {
  ...
  Store.deployed().then(function (contractInstance) {
    contractInstance.getWallet.call(address).then(function (balance) {
      $("#right-lower").html("<h3>Your transaction total :  $" + balance.toString() + "</h3>");
    });
  });
  Store.deployed().then(function (contractInstance) {
...
window.buy = function (url) {
  Store.deployed().then(function (contractInstance) {
    contractInstance.buy.sendTransaction(url, {
      gas: 1000000,
      from: web3.eth.defaultaAccount
    }).then(function (r) {
...
window.upload = function () {
  ...
  reader.onloadend = function () {
    ...
    ipfs.files.add(buf, (err, result) => { // Upload buffer to IPFS
      ...
      Store.deployed().then(async function (contractInstance) {
        try {
          var r = await contractInstance.addMedia.sendTransaction(url, mediaTitle, mediaPrice, {
            gas: 1000000,
            from: web3.eth.defaultaAccount
          });
        } catch (e) {
...
window.addToWallet = function () {
  ...
  Store.deployed().then(function (contractInstance) {
    contractInstance.addToWallet.sendTransaction(amtow, {
      gas: 1000000,
      from: web3.eth.defaultaAccount
    }).then(function (r) {
